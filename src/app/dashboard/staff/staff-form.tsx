...
export function StaffForm({ staff }: { staff: Staff[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-bold text-slate-700 mb-1.5">تاريخ التعيين (اختياري)</label>
        <input name="hire_date" type="date" className="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition" />
        <p className="text-xs text-slate-500 mt-1.5">تاريخ التحاق الموظف الفعلي بالعمل — قد يختلف عن تاريخ إدخاله بالنظام.</p>
      </div>
