...
import { Target, UserPlus, Save, ArrowRight, Plus, Trash2, Wrench, DoorClosed, Layers, Zap, Copy } from "lucide-react";
...
export function SalesOrderForm({ initialCustomerId, initialNewCustomer, submissionId, compact = false, onSubmitted }: SalesOrderFormProps) {
  ...
  // Form State
  const [isNewCustomer, setIsNewCustomer] = useState(!!initialNewCustomer);
  ...
  const duplicateLine = (id: string) => {
    ...
    if (!line) return;
    setLines([...lines, { ...line, id: Math.random().toString() }]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        if (onSubmitted && createdOrder?.id) {
          onSubmitted(createdOrder.id);
        } else {
  ...
  return (
    <div className={compact ? "" : "max-w-4xl mx-auto py-8"} dir="rtl">
      {!compact && (
          ...
          {!isNewCustomer ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
              ...
              <div>
                ...
                <input type="text" value={custCompanyName} onChange={e => setCustCompanyName(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition" placeholder="مثال: شركة النور للمقاولات" />
              </div>
            ...
            <div className="space-y-3">
              ...
              {lines.map((line) => line.type === 'slat' ? (
                <div key={line.id} className="bg-slate-50 p-5 rounded-2xl border border-teal-200 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={() => duplicateLine(line.id)} title="نسخ هذا الصنف" className="p-1.5 text-slate-400 hover:text-indigo-600 bg-white hover:bg-indigo-50 rounded-lg transition border border-slate-200">
                ...
                <div key={line.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start bg-slate-50 p-4 rounded-xl border border-slate-200">
                  ...
                  <div className="md:col-span-1 flex justify-center mt-2 md:mt-0 gap-1">
                    <button type="button" onClick={() => duplicateLine(line.id)} title="نسخ هذا الصنف" className="p-2 text-slate-400 hover:text-indigo-600 bg-white hover:bg-indigo-50 rounded-lg transition border border-slate-200 shadow-sm">
                      <Copy className="h-4 w-4" />
                    </button>
