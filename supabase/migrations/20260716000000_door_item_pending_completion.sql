-- يسمح بإدخال "مجرى فقط" (الارتفاع فقط) عند التركيب المبكر، على أن تُستكمل
-- بقية مواصفات الباب (العرض، اللون، الملحقات) لاحقاً على نفس السطر تماماً.
-- حالة صريحة بدل الاعتماد الضمني على "العرض فارغ = غير مكتمل".
ALTER TABLE erp_door_order_items ADD COLUMN IF NOT EXISTS item_status TEXT DEFAULT 'مكتمل'; -- 'قيد الاستكمال' | 'مكتمل'
ALTER TABLE erp_door_order_items ADD COLUMN IF NOT EXISTS initial_entry_date TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE erp_door_order_items ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;
-- ارتفاع المجرى وقت التركيب المبكر قد يختلف عن ارتفاع الباب النهائي؛
-- يُحفظ منفصلاً في guides_height_mm، بينما height_mm يبقى مخصصاً لارتفاع الباب الفعلي فقط.
ALTER TABLE erp_door_order_items ADD COLUMN IF NOT EXISTS guides_height_mm NUMERIC;
  ...
  FOR v_line IN SELECT * FROM erp_sales_order_lines WHERE sales_order_id = p_order_id LOOP
    ...
    ELSIF v_line.line_type = 'door' THEN
      ...
      IF v_door_order_id IS NULL THEN
        ...
        VALUES (v_order.customer_id, 'توريد', v_door_order_status, p_order_id)
