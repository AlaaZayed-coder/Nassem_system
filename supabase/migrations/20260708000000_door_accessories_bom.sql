-- إعادة هيكلة: الباب لم يعد يُدخل كطلبية منفصلة عن التصنيع، بل يبقى نفس
-- line_type='door' داخلياً (لا كسر لما هو مبني ومُختبر) لكن يُعرض في الواجهة
-- كخيار فرعي تحت "تصنيع". الإضافة الحقيقية هنا: دعم ملحقات الباب
-- (ريش/جبهة، ماتور) كبنية Parent-Child مرتبطة بنفس معاملة الاعتماد.

-- السماح بطلب شراء بدون صنف مُعرَّف مسبقاً في الكتالوج (صنف جديد يُكتب اسمه يدوياً)
ALTER TABLE erp_purchase_requests ALTER COLUMN item_code DROP NOT NULL;

CREATE TABLE IF NOT EXISTS erp_door_order_accessories (
  ...
  accessory_type TEXT NOT NULL, -- 'slat' | 'motor'
  item_code TEXT REFERENCES erp_items(item_code),
  free_text_name TEXT,
  quantity NUMERIC,
  fulfillment_status TEXT DEFAULT 'noted', -- 'noted' | 'available' | 'purchasing'
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM erp_sales_order_lines WHERE sales_order_id = p_order_id LOOP
    ...
    ELSIF v_line.line_type = 'door' THEN
      ...
      -- ===== ملحقات الباب (Parent-Child): ريش/جبهة وماتور =====
    ...
    ELSIF v_line.line_type = 'slat' THEN
      -- طلبيات الريش والجبهات المستقلة: تُنجز داخل قسم المبيعات نفسه
