-- طلبيات الريش والجبهات (Cube Sale Order): تُنجز داخل قسم المبيعات نفسه،
-- بدون ترحيل لقسم آخر (لا إنتاج ولا صيانة ولا شراء).
ALTER TABLE erp_sales_order_lines ADD COLUMN IF NOT EXISTS slat_specs JSONB;
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM erp_sales_order_lines WHERE sales_order_id = p_order_id LOOP
    ...
    ELSIF v_line.line_type = 'slat' THEN
      -- طلبيات الريش والجبهات: لا تُرحَّل لأي قسم آخر، تُعتبر مكتملة فور الاعتماد
